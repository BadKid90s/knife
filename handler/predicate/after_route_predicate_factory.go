...
func (f *AfterRoutePredicateFactory) Apply(definition Definition) Predicate[*web.ServerWebExchange] {
	config := f.parseConfig(definition)
	//return nil
	return f.apply(config)

}
func (f *AfterRoutePredicateFactory) parseConfig(definition Definition) *AfterPredicateConfig {
...
func (f *AfterRoutePredicateFactory) apply(config *AfterPredicateConfig) Predicate[*web.ServerWebExchange] {
